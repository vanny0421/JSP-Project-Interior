				...
				<div id="div_table">
							...
							<c:when test="${list != null and fn:length(list) > 0}">
								<c:forEach var="board" items="${list}">
									<tr align="center" valign="middle" onmouseover="this.style.backgroundColor='#e0f7fa'" onmouseout="this.style.backgroundColor=''">
										<td height="24px">${board.getBrag_board_no()}</td>
										<td><a href="${pageContext.request.contextPath}/Brag/Brag_BoardViewOk.br?brag_board_no=${board.getBrag_board_no()}">${board.getBrag_board_title()}</a></td>
										<td>${board.getUser_id()}</td>
										<td>${board.getBrag_board_date()}</td>
									</tr>
								</c:forEach>
							</c:when>
							<c:otherwise>
					...
					<table>
						<tr align="center" valign="middle">
							<td>
							<c:if test="${nowPage > 1}">
								<a href="${pageContext.request.contextPath}/Brag/Brag_BoardListOk.br?page=${nowPage-1}">&lt;</a>
							</c:if>
							...
							<c:forEach var="i" begin="${startPage}" end="${endPage}">
								<c:choose>
									...
									<c:otherwise>
										<a href="${pageContext.request.contextPath}/Brag/Brag_BoardList.br?page=${i}"><c:out value="${i}"/></a>
									</c:otherwise>
								</c:choose>
							</c:forEach>
							
							<c:if test="${nowPage != realEndPage}">
								<a href="${pageContext.request.contextPath}/Brag/Brag_BoardList.br?page=${nowPage+1}">&gt;</a>
							</c:if>
							</td>
						</tr>
					</table>
							...
							<c:choose>
								<c:when test="${session_id==null }">
									<div id="write_button_div">
										<a id="write" href="${pageContext.request.contextPath}/Brag/Brag_Write_Not_Signed_In.br" class="button">write</a>
									</div>
								</c:when>
							<c:otherwise>
					...
					<!-- 로그인 되어있을때 -->
					<div id="write_button_div">
					<a id="write" href="${pageContext.request.contextPath}/Brag/Brag_Write.br" class="button">write</a>
					</div>
							</c:otherwise>
						</c:choose>
				</div>
